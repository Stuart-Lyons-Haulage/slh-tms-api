...
if (!app.Environment.IsEnvironment("Testing"))
{
    ...
    try
    {
        ...
        if (trailerMerge.Renamed > 0 || trailerMerge.Merged > 0)
            logger.LogWarning(
                "Canonicalised trailer register: {Renamed} numeric trailers renamed, {Merged} duplicate aliases merged, {LoadsReassigned} loads and {MappingsReassigned} mappings reassigned.",
                trailerMerge.Renamed, trailerMerge.Merged, trailerMerge.LoadsReassigned, trailerMerge.MappingsReassigned);
