/*
   ...
   SQL is authoritative. This migration separates sender/customer identity from
   operational route rules, adds duplicate protection, and seeds only mappings
   that can be tied to an existing Customer master row.
*/
...
BEGIN
    ...
    IF COL_LENGTH(N'dbo.CustomerEmailRoutes', N'NormalizedMappingKey') IS NULL
    BEGIN
        ALTER TABLE dbo.CustomerEmailRoutes ADD NormalizedMappingKey AS
            LOWER(CONCAT(CustomerCode, N'|', ISNULL(SenderEmail, N''), N'|', ISNULL(SenderDomain, N''), N'|', ISNULL(SubjectContains, N''))) PERSISTED;
    END;

    IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'UX_CustomerEmailRoutes_Active_Normalized' AND object_id = OBJECT_ID(N'dbo.CustomerEmailRoutes'))
    BEGIN
        CREATE UNIQUE INDEX UX_CustomerEmailRoutes_Active_Normalized
            ON dbo.CustomerEmailRoutes (NormalizedMappingKey)
            WHERE Active = 1;
    END;
END;
...
BEGIN
    ...
    (
        ...
        NormalizedRuleKey AS LOWER(CONCAT(
            CustomerCode, N'|', ISNULL(OriginSiteCode, N''), N'|', ISNULL(OriginSiteName, N''), N'|',
            ISNULL(RetailerCode, N''), N'|', ISNULL(DestinationSiteCode, N''), N'|',
            ISNULL(DestinationCode, N''), N'|', ISNULL(DestinationName, N''), N'|', ISNULL(DestinationPostcode, N''))) PERSISTED,
        ...
        CONSTRAINT CK_OrderIntakeRouteRules_EffectiveDates CHECK (EffectiveTo IS NULL OR EffectiveFrom IS NULL OR EffectiveTo >= EffectiveFrom)
    );

    CREATE UNIQUE INDEX UX_OrderIntakeRouteRules_Active_Normalized
        ON dbo.OrderIntakeRouteRules (NormalizedRuleKey)
        WHERE Active = 1;

    CREATE INDEX IX_OrderIntakeRouteRules_Match
        ON dbo.OrderIntakeRouteRules (CustomerCode, Active, Priority)
        INCLUDE (OriginSiteCode, OriginSiteName, RetailerCode, DestinationSiteCode, DestinationCode, DestinationName, DestinationPostcode, ConfidenceScore, EffectiveFrom, EffectiveTo);
END;

/* Seed sender mappings only where a real SQL Customer master row exists. */
IF OBJECT_ID(N'dbo.CustomerEmailRoutes', N'U') IS NOT NULL AND OBJECT_ID(N'dbo.Customers', N'U') IS NOT NULL
BEGIN
    DECLARE @NwfCode nvarchar(40) = (SELECT TOP (1) Code FROM dbo.Customers WHERE Active = 1 AND (Code = N'NWF' OR Name LIKE N'%Natures Way%') ORDER BY CASE WHEN Code = N'NWF' THEN 0 ELSE 1 END, Code);
    DECLARE @BarfootsCode nvarchar(40) = (SELECT TOP (1) Code FROM dbo.Customers WHERE Active = 1 AND (Code = N'BARFOOTS' OR Name LIKE N'%Barfoots%') ORDER BY CASE WHEN Code = N'BARFOOTS' THEN 0 ELSE 1 END, Code);
    DECLARE @LangmeadCode nvarchar(40) = (SELECT TOP (1) Code FROM dbo.Customers WHERE Active = 1 AND (Code IN (N'LANGMEADS', N'LANGMEAD') OR Name LIKE N'%Langmead%') ORDER BY CASE WHEN Code = N'LANGMEADS' THEN 0 WHEN Code = N'LANGMEAD' THEN 1 ELSE 2 END, Code);
    DECLARE @SummerBerryCode nvarchar(40) = (SELECT TOP (1) Code FROM dbo.Customers WHERE Active = 1 AND (Code LIKE N'SUMMER%BERRY%' OR Name LIKE N'%Summer Berry%') ORDER BY Code);
    DECLARE @ApsCode nvarchar(40) = (SELECT TOP (1) Code FROM dbo.Customers WHERE Active = 1 AND (Code = N'APS' OR Name LIKE N'%APS%') ORDER BY CASE WHEN Code = N'APS' THEN 0 ELSE 1 END, Code);
    ...
    IF @SummerBerryCode IS NOT NULL AND NOT EXISTS (SELECT 1 FROM dbo.CustomerEmailRoutes WHERE Active = 1 AND CustomerCode = @SummerBerryCode AND SenderDomain = N'summerberry.co.uk' AND SubjectContains IS NULL)
    ...
    IF @ApsCode IS NOT NULL AND NOT EXISTS (SELECT 1 FROM dbo.CustomerEmailRoutes WHERE Active = 1 AND CustomerCode = @ApsCode AND SenderDomain = N'apsgroup.uk.com' AND SubjectContains IS NULL)
...
/* Route examples are intentionally non-exclusive and remain planner-reviewable. */
