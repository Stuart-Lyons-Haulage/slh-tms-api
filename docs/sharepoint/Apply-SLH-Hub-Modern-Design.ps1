...
# Apply a clean modern header and footer where supported.
try {
...
if(-not $page){
  ...
  # Keep the page identity stable but rebuild the canvas to make repeat runs deterministic.
  $page.Controls.Clear()
  $page.Sections.Clear()
}
...
# Hero links are implemented with Quick Links for portability across tenants.
$quickLinksProps = @{
  layoutId = 'Button'
