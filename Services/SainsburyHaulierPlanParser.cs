...
public sealed class SainsburyHaulierPlanParser
{
    ...
    public EmailIntakeParseResult? TryParse(MailboxEmailIntakeRequest request)
    {
        ...
        try
        {
            ...
            do
            {
                ...
                if (collectIndex < 0 || deliverIndex < 0 || collectDateIndex < 0 || deliveryDateIndex < 0)
                    continue;
            ...
            if (orders.Count == 0)
            {
                return new EmailIntakeParseResult(
                    [],
                    ["Sainsbury Haulier Plan workbook was read but no populated STUART LYONS rows were found."],
                    "No Stuart Lyons transport rows were found in the attached Sainsbury plan.");
            }
        ...
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            return new EmailIntakeParseResult(
                [],
                [$"Sainsbury Haulier Plan could not be parsed: {ex.GetBaseException().Message}"],
                "Sainsbury workbook parsing failed; manual review is required.");
        }
    }
    ...
    private static bool IsHeader(object?[] row)
    {
        ...
        return values.Contains("collectfrom") &&
               values.Contains("deliverto") &&
               values.Contains("targetcollectiondatetime") &&
               values.Contains("targetdeliverydatetime");
    }
    ...
    private static string? CellText(object? value) =>
        value is null || value is DBNull ? null : Convert.ToString(value, CultureInfo.InvariantCulture)?.Trim();
    ...
    private static bool IsTemplateValue(string? value) =>
        string.IsNullOrWhiteSpace(value) || string.Equals(value.Trim(), "27", StringComparison.OrdinalIgnoreCase);
