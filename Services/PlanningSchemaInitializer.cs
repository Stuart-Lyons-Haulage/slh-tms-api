...
public static class PlanningSchemaInitializer
{
    public static async Task Apply(TmsDbContext db, CancellationToken ct)
    {
        ...
        foreach (var resourceName in new[] { "Slh.Tms.Api.Database.001_Initial_Tms_Schema.sql", "Slh.Tms.Api.Database.002_Planning_Loads_Schema.sql", "Slh.Tms.Api.Database.003_Market_Order_Details.sql", "Slh.Tms.Api.Database.004_Driver_Mobile_Number.sql", "Slh.Tms.Api.Database.005_Delivery_Windows.sql", "Slh.Tms.Api.Database.006_Customer_Contacts.sql" })
        {
