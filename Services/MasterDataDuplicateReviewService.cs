...
public static class MasterDataDuplicateReviewService
{
    ...
    private static async Task<IReadOnlyList<MasterDataDuplicateCandidate>> ExcludeRejectedCandidatesAsync(TmsDbContext db, IReadOnlyList<MasterDataDuplicateCandidate> candidates, CancellationToken ct)
    {
        ...
        var rejectedIds = await db.MasterDataAudits.AsNoTracking()
        ...
        return rejected.Count == 0
            ? candidates
            : candidates.Where(candidate => !rejected.Contains(candidate.CandidateId)).ToList();
    }
    ...
    private static async Task<IReadOnlyList<MasterDataDuplicateCandidate>> FindSiteCandidatesAsync(TmsDbContext db, CancellationToken ct)
    {
        ...
        {
            var code = Normalise(site.ExternalCode);
            var name = Normalise(site.Name);
            var driverTextName = Normalise(site.DriverTextName);
            var address = NormaliseAddress(site.CollectionAddress);
            var postcode = ExtractPostcode(site.CollectionAddress);

            if (code.Length >= 2) Add(byKey, $"external-code:{code}", site);
            if (name.Length >= 5 && !string.IsNullOrWhiteSpace(postcode)) Add(byKey, $"name-postcode:{name}|{postcode}", site);
            if (driverTextName.Length >= 5 && !string.IsNullOrWhiteSpace(postcode)) Add(byKey, $"driver-name-postcode:{driverTextName}|{postcode}", site);
            ...
            if (name.Length >= 8 && string.IsNullOrWhiteSpace(postcode) && address.Length == 0) Add(byKey, $"name-only-missing-address:{name}", site);
        }
        return DistinctGroups(byKey.Values, site => site.Id)
    ...
    private static async Task<IReadOnlyList<MasterDataDuplicateCandidate>> FindDriverCandidatesAsync(TmsDbContext db, CancellationToken ct)
    {
        ...
        var byKey = new Dictionary<string, HashSet<Driver>>(StringComparer.OrdinalIgnoreCase);

        foreach (var driver in rows)
        {
            var tacho = Normalise(driver.TachoMasterDriverId);
            var employee = Normalise(driver.EmployeeNumber);
            var licence = Normalise(driver.DrivingLicenceNumber);
            var name = Normalise(driver.DisplayName);

            if (tacho.Length > 0) Add(byKey, $"tachomaster:{tacho}", driver);
            if (employee.Length > 0) Add(byKey, $"employee:{employee}", driver);
            if (licence.Length > 6) Add(byKey, $"licence:{licence}", driver);
            if (name.Length >= 8 && !string.IsNullOrWhiteSpace(driver.MobileNumber)) Add(byKey, $"name-mobile:{name}|{Normalise(driver.MobileNumber)}", driver);
        }

        return DistinctGroups(byKey.Values, driver => driver.Id)
    ...
    private static async Task<IReadOnlyList<MasterDataDuplicateCandidate>> FindVehicleCandidatesAsync(TmsDbContext db, CancellationToken ct)
    {
        ...
        var byKey = new Dictionary<string, HashSet<Vehicle>>(StringComparer.OrdinalIgnoreCase);

        foreach (var vehicle in rows)
        {
            var registration = NormaliseRegistration(vehicle.Registration);
            var fleetioId = Normalise(vehicle.FleetioId);
            var vin = Normalise(vehicle.VIN);

            if (registration.Length > 0) Add(byKey, $"registration:{registration}", vehicle);
            if (fleetioId.Length > 0) Add(byKey, $"fleetio:{fleetioId}", vehicle);
            if (vin.Length > 8) Add(byKey, $"vin:{vin}", vehicle);
        }

        return DistinctGroups(byKey.Values, vehicle => vehicle.Id)
    ...
    private static async Task<IReadOnlyList<MasterDataDuplicateCandidate>> FindTrailerCandidatesAsync(TmsDbContext db, CancellationToken ct)
    {
        ...
        var byKey = new Dictionary<string, HashSet<Trailer>>(StringComparer.OrdinalIgnoreCase);

        foreach (var trailer in rows)
        {
            var exact = NormaliseRegistration(trailer.TrailerNumber);
            var canonical = NormaliseTrailerNumber(trailer.TrailerNumber);

            if (exact.Length > 0) Add(byKey, $"trailer:{exact}", trailer);
            if (canonical.Length > 0) Add(byKey, $"trailer-canonical:{canonical}", trailer);
        }

        return DistinctGroups(byKey.Values, trailer => trailer.Id)
    ...
    private static async Task<IReadOnlyList<MasterDataDuplicateCandidate>> FindMarketCandidatesAsync(TmsDbContext db, CancellationToken ct)
    {
        ...
        var byKey = new Dictionary<string, HashSet<MarketContact>>(StringComparer.OrdinalIgnoreCase);
        ...
        foreach (var row in rows)
        {
            ...
            if (market.Length > 0 && name.Length > 0) Add(byKey, $"market-name-stand:{market}|{name}|{stand}", row);
            if (market.Length > 0 && sender.Length > 0) Add(byKey, $"market-sender:{market}|{sender}", row);
        }

        return DistinctGroups(byKey.Values, row => row.Id)
    ...
    {
        var canonical = await db.Sites.FirstOrDefaultAsync(x => x.Id == request.CanonicalId && x.Active, ct);
        if (canonical is null) return new MasterDataDuplicateMergeResult(0, 0, ["Canonical site was not found."]);
        var duplicates = await db.Sites.Where(x => request.DuplicateIds.Contains(x.Id) && x.Id != canonical.Id && x.Active).ToListAsync(ct);
        ...
        var geofences = await db.SiteGeofences.Where(x => duplicateIds.Contains(x.SiteId)).ToListAsync(ct);
        foreach (var geofence in geofences) geofence.SiteId = canonical.Id;

        var runStops = await db.RunStops.Where(x => duplicateIds.Contains(x.SiteId)).ToListAsync(ct);
        ...
        db.MasterDataAudits.Add(new MasterDataAudit { EntityType = "Site", EntityId = canonical.Id, Action = "DuplicateMerge", ChangedBy = actor, ChangesJson = JsonSerializer.Serialize(new { canonical = canonical.ExternalCode, merged = duplicates.Select(x => x.ExternalCode), request.Note, geofencesReassigned = geofences.Count, runStopsReassigned = runStops.Count, mappingsReassigned = mappingCount, messages }) });
        await db.SaveChangesAsync(ct);
    ...
    {
        var canonical = await db.Drivers.FirstOrDefaultAsync(x => x.Id == request.CanonicalId && x.Active, ct);
        if (canonical is null) return new MasterDataDuplicateMergeResult(0, 0, ["Canonical driver was not found."]);
        var duplicates = await db.Drivers.Where(x => request.DuplicateIds.Contains(x.Id) && x.Id != canonical.Id && x.Active).ToListAsync(ct);
        ...
        var loads = await db.Loads.Where(x => x.DriverId.HasValue && duplicateIds.Contains(x.DriverId.Value)).ToListAsync(ct);
        foreach (var load in loads) load.DriverId = canonical.Id;

        var planRuns = await db.PlanProposalRuns.Where(x => x.DriverId.HasValue && duplicateIds.Contains(x.DriverId.Value)).ToListAsync(ct);
        foreach (var run in planRuns) run.DriverId = canonical.Id;

        var candidates = await db.PlanProposalCandidates.Where(x => duplicateIds.Contains(x.DriverId)).ToListAsync(ct);
        foreach (var candidate in candidates) candidate.DriverId = canonical.Id;

        var runResources = await db.RunResourceAllocations.Where(x => duplicateIds.Contains(x.DriverId)).ToListAsync(ct);
        foreach (var allocation in runResources) allocation.DriverId = canonical.Id;

        var statusLogs = await db.DriverStatusLogs.Where(x => x.DriverId.HasValue && duplicateIds.Contains(x.DriverId.Value)).ToListAsync(ct);
        ...
        db.MasterDataAudits.Add(new MasterDataAudit { EntityType = "Driver", EntityId = canonical.Id, Action = "DuplicateMerge", ChangedBy = actor, ChangesJson = JsonSerializer.Serialize(new { canonical = canonical.EmployeeNumber, merged = duplicates.Select(x => x.EmployeeNumber), request.Note, loadsReassigned = loads.Count, planRunsReassigned = planRuns.Count, candidatesReassigned = candidates.Count, runResourcesReassigned = runResources.Count, statusLogsReassigned = statusLogs.Count, mappingsReassigned = mappings }) });
        await db.SaveChangesAsync(ct);
    ...
    {
        var canonical = await db.Vehicles.FirstOrDefaultAsync(x => x.Id == request.CanonicalId && x.Active, ct);
        if (canonical is null) return new MasterDataDuplicateMergeResult(0, 0, ["Canonical vehicle was not found."]);
        var duplicates = await db.Vehicles.Where(x => request.DuplicateIds.Contains(x.Id) && x.Id != canonical.Id && x.Active).ToListAsync(ct);
        ...
        var loads = await db.Loads.Where(x => x.VehicleId.HasValue && duplicateIds.Contains(x.VehicleId.Value)).ToListAsync(ct);
        foreach (var load in loads) load.VehicleId = canonical.Id;

        var planRuns = await db.PlanProposalRuns.Where(x => x.VehicleId.HasValue && duplicateIds.Contains(x.VehicleId.Value)).ToListAsync(ct);
        foreach (var run in planRuns) run.VehicleId = canonical.Id;

        var candidates = await db.PlanProposalCandidates.Where(x => duplicateIds.Contains(x.VehicleId)).ToListAsync(ct);
        foreach (var candidate in candidates) candidate.VehicleId = canonical.Id;

        var runResources = await db.RunResourceAllocations.Where(x => duplicateIds.Contains(x.VehicleId)).ToListAsync(ct);
        ...
        db.MasterDataAudits.Add(new MasterDataAudit { EntityType = "Vehicle", EntityId = canonical.Id, Action = "DuplicateMerge", ChangedBy = actor, ChangesJson = JsonSerializer.Serialize(new { canonical = canonical.Registration, merged = duplicates.Select(x => x.Registration), request.Note, loadsReassigned = loads.Count, planRunsReassigned = planRuns.Count, candidatesReassigned = candidates.Count, runResourcesReassigned = runResources.Count, mappingsReassigned = mappings }) });
        await db.SaveChangesAsync(ct);
    ...
    {
        var canonical = await db.Trailers.FirstOrDefaultAsync(x => x.Id == request.CanonicalId && x.Active, ct);
        if (canonical is null) return new MasterDataDuplicateMergeResult(0, 0, ["Canonical trailer was not found."]);
        var duplicates = await db.Trailers.Where(x => request.DuplicateIds.Contains(x.Id) && x.Id != canonical.Id && x.Active).ToListAsync(ct);
        ...
        var loads = await db.Loads.Where(x => x.TrailerId.HasValue && duplicateIds.Contains(x.TrailerId.Value)).ToListAsync(ct);
        foreach (var load in loads) load.TrailerId = canonical.Id;

        var planRuns = await db.PlanProposalRuns.Where(x => x.TrailerId.HasValue && duplicateIds.Contains(x.TrailerId.Value)).ToListAsync(ct);
        foreach (var run in planRuns) run.TrailerId = canonical.Id;

        var runResources = await db.RunResourceAllocations.Where(x => duplicateIds.Contains(x.TrailerId)).ToListAsync(ct);
        ...
        db.MasterDataAudits.Add(new MasterDataAudit { EntityType = "Trailer", EntityId = canonical.Id, Action = "DuplicateMerge", ChangedBy = actor, ChangesJson = JsonSerializer.Serialize(new { canonical = canonical.TrailerNumber, merged = duplicates.Select(x => x.TrailerNumber), request.Note, loadsReassigned = loads.Count, planRunsReassigned = planRuns.Count, runResourcesReassigned = runResources.Count, mappingsReassigned = mappings }) });
        await db.SaveChangesAsync(ct);
    ...
    {
        var canonical = await db.MarketContacts.FirstOrDefaultAsync(x => x.Id == request.CanonicalId && x.Active, ct);
        if (canonical is null) return new MasterDataDuplicateMergeResult(0, 0, ["Canonical market record was not found."]);
        var duplicates = await db.MarketContacts.Where(x => request.DuplicateIds.Contains(x.Id) && x.Id != canonical.Id && x.Active).ToListAsync(ct);
    ...
    private static MasterDataDuplicateCandidate BuildSiteCandidate(IReadOnlyList<Site> group)
    {
        ...
        var exactExternalCode = duplicates.Any(x => Normalise(x.ExternalCode) == Normalise(canonical.ExternalCode));
        var exactName = duplicates.All(x => Normalise(x.Name) == Normalise(canonical.Name) || Normalise(x.DriverTextName) == Normalise(canonical.Name) || Normalise(x.Name) == Normalise(canonical.DriverTextName));
        var samePostcode = !string.IsNullOrWhiteSpace(postcode) && duplicates.All(x => ExtractPostcode(x.CollectionAddress) == postcode);
        var confidence = exactExternalCode && exactName ? 99 : exactExternalCode ? 94 : exactName && samePostcode ? 98 : exactName ? 86 : samePostcode ? 82 : 70;
        return new MasterDataDuplicateCandidate(
            CandidateId($"site:{canonical.Id}:{string.Join(',', duplicates.Select(x => x.Id))}"),
            "sites",
            confidence,
            exactExternalCode ? "Same site external code; merge preserves address/routing data." : samePostcode ? "Same normalised site name/address and postcode." : "Likely duplicate site name/address. Review before merging.",
            confidence >= 95,
    ...
    {
        var canonical = group.OrderByDescending(x => new[] { x.TachoMasterDriverId, x.MobileNumber, x.DriverType, x.DriverGroup, x.Skills, x.DrivingLicenceNumber }.Count(v => !string.IsNullOrWhiteSpace(v))).ThenBy(x => x.DisplayName).First();
        var duplicates = group.Where(x => x.Id != canonical.Id).ToList();
        var sameTacho = !string.IsNullOrWhiteSpace(canonical.TachoMasterDriverId) && duplicates.Any(x => Normalise(x.TachoMasterDriverId) == Normalise(canonical.TachoMasterDriverId));
        var sameEmployee = !string.IsNullOrWhiteSpace(canonical.EmployeeNumber) && duplicates.Any(x => Normalise(x.EmployeeNumber) == Normalise(canonical.EmployeeNumber));
        var confidence = sameTacho ? 99 : sameEmployee ? 96 : 90;
        return new MasterDataDuplicateCandidate(CandidateId($"driver:{canonical.Id}:{string.Join(',', duplicates.Select(x => x.Id))}"), "drivers", confidence, sameTacho ? "Same Tachomaster member number." : sameEmployee ? "Same employee number across driver records." : "Likely duplicate driver identity.", confidence >= 95, DriverRecord(canonical), duplicates.Select(DriverRecord).ToList(), ["tachomasterDriverId", "mobileNumber", "driverType", "driverGroup", "skills", "linked loads/runs"]);
    }

    private static MasterDataDuplicateCandidate BuildVehicleCandidate(IReadOnlyList<Vehicle> group)
    {
        var canonical = group.OrderByDescending(x => new[] { x.FleetNumber, x.Abbreviation, x.FuelProvider, x.CabMobile, x.FuelPin, x.ShellCard, x.BpRedCard, x.BpPlainCard, x.FleetioId }.Count(v => !string.IsNullOrWhiteSpace(v))).ThenBy(x => x.Registration).First();
        var duplicates = group.Where(x => x.Id != canonical.Id).ToList();
        return new MasterDataDuplicateCandidate(CandidateId($"vehicle:{canonical.Id}:{string.Join(',', duplicates.Select(x => x.Id))}"), "vehicles", 99, "Same normalised vehicle registration, VIN or Fleetio identity.", true, VehicleRecord(canonical), duplicates.Select(VehicleRecord).ToList(), ["fleetNumber", "abbreviation", "fuelProvider", "cabMobile", "fuelPin", "fuel cards", "Fleetio fields", "linked loads/runs"]);
    }
    ...
    private static MasterDataDuplicateCandidate BuildTrailerCandidate(IReadOnlyList<Trailer> group)
    {
        ...
        var duplicates = group.Where(x => x.Id != canonical.Id).ToList();
        return new MasterDataDuplicateCandidate(CandidateId($"trailer:{canonical.Id}:{string.Join(',', duplicates.Select(x => x.Id))}"), "trailers", 99, "Same trailer number after normalising SLH/numeric aliases.", true, TrailerRecord(canonical), duplicates.Select(TrailerRecord).ToList(), ["type", "standardCapacity", "euroCapacity", "linked loads/runs"]);
    }
    ...
    private static MasterDataDuplicateCandidate BuildMarketCandidate(IReadOnlyList<MarketContact> group)
    {
        ...
        var duplicates = group.Where(x => x.Id != canonical.Id).ToList();
        var sameSender = !string.IsNullOrWhiteSpace(canonical.Sender) && duplicates.Any(x => Normalise(x.Sender) == Normalise(canonical.Sender));
        return new MasterDataDuplicateCandidate(CandidateId($"market:{canonical.Id}:{string.Join(',', duplicates.Select(x => x.Id))}"), "markets", sameSender ? 96 : 94, sameSender ? "Same market sender/contact identity." : "Same market, customer/sender and stand/location.", sameSender, MarketRecord(canonical), duplicates.Select(MarketRecord).ToList(), ["standOrLocation", "salesman", "sender"]);
    }
    ...
    private static IReadOnlyList<IReadOnlyList<T>> DistinctGroups<T>(IEnumerable<HashSet<T>> source, Func<T, Guid> id) where T : notnull
    {
        ...
        return source.Select(group => group.OrderBy(id).ToList())
            .Where(group => group.Count > 1 && seen.Add(string.Join('|', group.Select(id).OrderBy(x => x))))
    ...
    private static async Task<int> ReassignIntegrationMappingsAsync(TmsDbContext db, string entityType, IReadOnlyCollection<Guid> duplicateIds, Guid canonicalId, string actor, CancellationToken ct)
    {
        var mappings = await db.IntegrationMappings
            .Where(x => x.TmsEntityType == entityType && duplicateIds.Contains(x.TmsEntityId))
            .ToListAsync(ct);
    ...
    private static int SiteCompleteness(Site site) => new object?[] { site.CollectionAddress, site.MapLink, site.Latitude, site.Longitude, site.CollectionInstructions, site.DriverTextName, site.Aliases, site.OperationalRegion, site.CustomerCode }.Count(x => x is not null && !string.IsNullOrWhiteSpace(x.ToString()));
