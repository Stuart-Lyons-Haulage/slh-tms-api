        ...
        StagedImport item, JsonElement payload, string reference, string customerCode, CancellationToken ct)
    {
        ...
        for (var index = 0; index < sourceLines.Count; index++)
        {
            ...
            db.OrderSourceLines.Add(new OrderSourceLine
            {
                ...
                CollectionTimeFrom = TimeOnlyOrNull(line, "collectionTimeFrom") ?? TimeOnlyOrNull(payload, "collectionTimeFrom") ?? ParseTimingTime(masterRule?.CollectFrom),
                CollectionTimeTo = TimeOnlyOrNull(line, "collectionTimeTo") ?? TimeOnlyOrNull(payload, "collectionTimeTo") ?? ParseTimingTime(masterRule?.CollectTo),
