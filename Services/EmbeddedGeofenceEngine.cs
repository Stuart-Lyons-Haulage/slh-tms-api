...
public static class EmbeddedGeofenceEngine
{
    ...
    public static async Task<EmbeddedGeofenceSnapshot> BuildAsync(TmsDbContext db, DateOnly planningDate, IReadOnlyCollection<Load> loads, CancellationToken ct)
    {
        ...
        foreach (var vehicle in vehicles)
        {
            ...
            visits.AddRange(DeriveVisits(vehicle.Id, vehicle.Registration, vehicleEvents.OrderBy(x => x.EventTimeUtc).ToList(), progressionFences));
        }
        ...
        var latestTracking = new[]
            ...
            .DefaultIfEmpty()
            .Max();
        return new EmbeddedGeofenceSnapshot(allFences, visits, activeVisits, confirmed, observationCount, latestTracking == default ? null : latestTracking);
    }
