...
public static class MasterDetailStore
{
    ...
    public static async Task<TrailerAliasMergeResult> MergeSlhTrailerAliasesAsync(TmsDbContext db, CancellationToken ct)
    {
        ...
        if (renamed > 0 || merged > 0 || reassignedLoads > 0 || reassignedMappings > 0)
            await db.SaveChangesAsync(ct);

        return new TrailerAliasMergeResult(renamed, merged, reassignedLoads, reassignedMappings);
    }
...
public sealed record TrailerAliasMergeResult(int Renamed, int Merged, int LoadsReassigned, int MappingsReassigned);