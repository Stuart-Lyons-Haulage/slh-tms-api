...
public sealed class SchemaResourceTests
{
    ...
    public void Schema_migration_catalog_versions_every_embedded_database_script_once()
    {
        ...
        Assert.Equal("037_Driver_Tacho_Identity.sql", migrations[^19].Name);
        Assert.Equal("038_Driver_Tacho_Identity_Repair.sql", migrations[^18].Name);
        Assert.Equal("039_Canonical_Relational_Planning.sql", migrations[^17].Name);
        Assert.Equal("040_Audit_Outbox.sql", migrations[^16].Name);
        Assert.Equal("041_Distributed_Integration_Lease.sql", migrations[^15].Name);
        Assert.Equal("042_Operational_Read_Performance_Indexes.sql", migrations[^14].Name);
        Assert.Equal("043_Customer_Site_Crm_Links.sql", migrations[^13].Name);
        Assert.Equal("044_Market_Read_Only_Map.sql", migrations[^12].Name);
        Assert.Equal("045_Customer_Contacts_Master_Projection.sql", migrations[^11].Name);
        Assert.Equal("046_Driver_Card_Read_And_Source_Detail.sql", migrations[^10].Name);
        Assert.Equal("047_Market_Seller_Stand_Duplicates.sql", migrations[^9].Name);
        Assert.Equal("048_Customer_Email_Route_Market_Key.sql", migrations[^8].Name);
        Assert.Equal("049_Market_Contact_Stable_Key_And_Stands.sql", migrations[^7].Name);
        Assert.Equal("050_Master_Vehicle_Source_Detail.sql", migrations[^6].Name);
        Assert.Equal("051_Vehicle_Source_Detail.sql", migrations[^5].Name);
        Assert.Equal("052_Email_Intake_Fast_Path.sql", migrations[^4].Name);
        Assert.Equal("058_Operational_Compliance_Fields.sql", migrations[^3].Name);
        Assert.Equal("059_RoadTech_Operational_Visits.sql", migrations[^2].Name);
        Assert.Equal("060_TachoMaster_Job_Managed_Identity.sql", migrations[^1].Name);
    }
