...
public sealed class OrderLifecycleEndToEndTests : IClassFixture<CustomWebFactory>
{
    ...
    public async Task Raw_email_is_staged_promoted_allocated_and_retains_quantity_end_to_end()
    {
        ...
        var intake = await PostJson(client, "/api/v1/order-intake/email", MailboxOrder(
            messageId, po, 4, planningDate, planningDate.AddDays(1)));
    ...
    public async Task Revised_email_creates_new_revision_not_duplicate_and_exposes_affected_run_allocation()
    {
        ...
        Assert.Equal(HttpStatusCode.OK,
            (await PostJson(client, $"/api/v1/staging/{originalStagedId}/approve", new { note = "Original approved" })).StatusCode);
        ...
        var originalAllocation = await PostJson(client, "/api/v1/planning-control/allocations", new
        {
            orderId = order.Id,
            loadId,
            date = planningDate,
        ...
        Assert.Equal(HttpStatusCode.OK, originalAllocation.StatusCode);
        ...
        Assert.Equal(HttpStatusCode.OK,
            (await PostJson(client, $"/api/v1/staging/{revisedStagedId}/approve", new { note = "Revision approved" })).StatusCode);

        using (var scope = factory.Services.CreateScope())
        {
            var db = scope.ServiceProvider.GetRequiredService<TmsDbContext>();
            var movement = Assert.Single(await db.OrderMovements.Where(x => x.Id == order.SourceMovementId).ToListAsync());
            var revisions = await db.OrderRevisions
                .Where(x => x.MovementId == movement.Id)
        ...
        // The live order now requires seven units, but the existing run still holds the original four.
        // Pallet Control therefore exposes a three-unit outstanding delta against the allocated run.
        ...
        Assert.Contains(orderRow.GetProperty("allocations").EnumerateArray(),
            allocation => allocation.GetProperty("loadId").GetGuid() == loadId && allocation.GetProperty("pallets").GetInt32() == 4);
    }
